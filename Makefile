...
test-render:        ## Render fixtures under STYLE (default: every shipped style, no API call)
	@if [ -n "$(STYLE)" ]; then \
...
golden-update:      ## Regenerate tests/golden/test-render-<style>.html for every shipped style
	@for s in editorial; do \
		$(PY) -c "from tests.conftest import render_with_fixtures; from trailstory.models import Style; render_with_fixtures(style=Style('$$s'))"; \
